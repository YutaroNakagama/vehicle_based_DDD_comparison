...
for batch_size in batch_sizes:
    for epoch in epochs:
        ...
        for train_index, test_index in kf.split(X_data_3d):
            ...
            model.fit(X_train, y_train, epochs=epoch, batch_size=batch_size, validation_split=0.2, verbose=1, callbacks=[early_stopping])
            end_time = time.time()
...
for train_index, test_index in kf.split(X_data_3d):
    ...
    final_model.fit(X_train, y_train, epochs=best_params['epochs'], batch_size=best_params['batch_size'], validation_split=0.2, verbose=1)
    ...
    roc_data.to_csv(f'roc_curve_fold_{fold_idx+1}.csv', index=False)
    print(f"ROCデータを保存しました: roc_curve_fold_{fold_idx+1}.csv")
