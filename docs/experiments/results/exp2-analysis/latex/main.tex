...
Zhu et~al.~\cite{zhu2025} proposed MADFM, a multi-layer adaptive fatigue monitoring model using steering wheel signals in IEEE T-ITS.
Perkins et~al.~\cite{perkins2022} identified remaining implementation challenges for drowsiness prediction systems in IEEE T-IV.
Recent surveys~\cite{fu2024,fonseca2025,alquraishi2024} confirm that vehicle-dynamics-based methods remain a practical backbone, but note that most studies evaluate a single architecture on a fixed data split without systematically examining the effect of preprocessing decisions.
...
Ganguly et~al.~\cite{ganguly2025} proposed an attention-based deep learning framework for drowsiness detection, and Ali et~al.~\cite{ali2026} provided a comprehensive T-ITS review of brain-imaging-based drowsiness detection.
...
Distance and Membership remain negligible ($S_D^{\mathrm{XGB}} = 0.002$, $S_G^{\mathrm{XGB}} = 0.011$).
The systematic concentration ratio $\frac{S_M + S_R + S_{R\times M}}{1 - S_{\varepsilon}} = 92.2\%$ (XGB) remains above 90\%, confirming that the dominance of $M$, $R$, and $R\times M$ is not an artifact of the RF learning algorithm.
...
\begin{table}[!t]
  ...
  \begin{tabular}{l c c}
    ...
    $R \times M$        & 0.212 & 0.112 \\
    Distance ($D$)      & 0.005 & 0.002 \\
    Membership ($G$)    & 0.010 & 0.011 \\
    Residual ($\varepsilon$) & 0.157 & 0.080 \\
    \bottomrule
  \end{tabular}
\end{table}

The Within--Mixed equivalence replicates: SMOTE-based strategies achieve the best F2 (SW-SMOTE $r\!=\!0.1$: 0.758 within; SMOTE $r\!=\!0.1$: 0.790 mixed), and the Spearman rank correlation between within-domain and mixed mode strategy rankings is $\rho = 0.96$ ($p < 0.001$).
However, the cross-domain ranking reversal observed with RF does \emph{not} replicate: SMOTE variants remain the top-ranked strategies even in cross-domain mode (F2\,$= 0.18$--$0.22$), yielding $\rho_{\mathrm{cross,within}} = {+}0.71$ ($p = 0.07$) rather than the negative correlation seen with RF ($\rho = -0.74$ to $-0.89$).
This attenuation is consistent with the smaller $R\times M$ interaction in XGBoost ($S_{R\times M} = 0.112$ vs.~$0.212$ for RF), suggesting that gradient boosting's sequential residual correction partially compensates for the source--target mismatch that penalises SMOTE under RF bagging.
...
\begin{enumerate}
  ...
  Concretely, each technique can be introduced as an additional factor level (e.g., $M' \in \{\text{cross, within, mixed, DA-aligned, few-shot}\}$) and evaluated through the same Sobol decomposition pipeline with no methodological change, yielding directly comparable variance shares.
\end{enumerate}
...
\begin{table*}[!t]
  ...
  \begin{tabular}{l c l l c c c c}
    ...
    Ganguly et~al.~\cite{ganguly2025}           & 2025 & Attn-DL   & EEG     & --- & 0           & No               & --- \\
    ...
    Fu et~al.~\cite{fu2024}                    & 2024 & Survey    & Multi   & --- & ---          & ---              & --- \\
    Perkins et~al.~\cite{perkins2022}          & 2022 & Survey    & Multi   & --- & ---          & ---              & --- \\
...
\begin{enumerate}
  ...
  While both classifiers are ensemble methods operating on tabular features, their inductive biases differ fundamentally: RF reduces variance via bootstrap aggregation and random feature subsets, whereas XGBoost reduces bias through sequential residual correction with regularisation.
  The Sobol--Hoeffding decomposition framework itself is classifier-agnostic---it treats the classifier as a black box mapping factor configurations to performance scores---and can therefore be extended to sequence models (LSTM~\cite{wang2022}), attention-based architectures~\cite{ganguly2025}, or any other learner without methodological modification.
...
\begin{enumerate}
  ...
  $M$ ($S_{TM} = 0.48$--$0.66$) and $R$ ($S_{TR} = 0.40$--$0.46$), including their interaction ($S_{R\times M} = 0.11$--$0.21$), account for ${>}\,92$\% of systematic variance (97.5\% RF, 92.2\% XGBoost).
  SW-SMOTE $r\!=\!0.1$ achieves the best within-domain performance (F2\,=\,0.558, AUROC\,=\,0.903 for RF; F2\,=\,0.758, AUROC\,=\,0.877 for XGBoost).

  \item \textbf{A strong interaction causes strategy ranking reversal (RF).}
  The optimal rebalancing strategy depends on training mode in RF: RUS is effective only in cross-domain settings, while SMOTE dominates in within-domain and mixed modes ($\rho_{\mathrm{cross,within}} = -0.74$ to $-0.89$).
  With XGBoost, the interaction is attenuated ($S_{R\times M} = 0.11$ vs.~$0.21$) and SMOTE variants rank highly even in cross-domain mode ($\rho_{\mathrm{cross,within}} = {+}0.71$), indicating that the reversal is classifier-dependent while the factor hierarchy is not.
...
\bibitem{zhu2025}
J.~Zhu, Q.~Liu, Z.~Zhao, Y.~Lu, Z.~Yao, and Y.~Zhang, ``MADFM: Multi-layer adaptive driver fatigue monitoring model based on steering wheel signals,'' \emph{IEEE Trans. Intell. Transp. Syst.}, 2025.

\bibitem{fu2024}
B.~Fu, F.~Boutros, C.-T.~Lin, and N.~Damer, ``A survey on drowsiness detection---modern applications and methods,'' \emph{IEEE Trans. Intell. Veh.}, vol.~9, no.~5, pp.~5210--5234, 2024.

\bibitem{perkins2022}
E.~Perkins, C.~Sitaula, M.~Burke, and F.~Dehzangi, ``Challenges of driver drowsiness prediction: The remaining steps to implementation,'' \emph{IEEE Trans. Intell. Veh.}, vol.~8, no.~12, pp.~4704--4716, 2022.

\bibitem{ali2026}
M.~U.~Ali, A.~Zafar, S.~H.~Kim, and K.~S.~Kim, ``Intelligent driver drowsiness detection using brain imaging: A systematic review for enhanced road safety in intelligent transportation systems,'' \emph{IEEE Trans. Intell. Transp. Syst.}, 2026.

\bibitem{ganguly2025}
B.~Ganguly, D.~Dey, and S.~Munshi, ``An attention deep learning framework-based drowsiness detection model for intelligent transportation system,'' \emph{IEEE Trans. Intell. Transp. Syst.}, vol.~26, no.~4, pp.~4917--4928, 2025.

\bibitem{hu2022}
Z.~Hu, S.~Lou, Y.~Xing, X.~Wang, D.~Cao, and C.~Lv, ``Review and perspectives on driver digital twin and its enabling technologies for intelligent vehicles,'' \emph{IEEE Trans. Intell. Veh.}, vol.~7, no.~3, pp.~417--440, 2022.

\bibitem{fang2024}
Z.~Fang, J.~Wang, Z.~Wang, J.~Chen, and J.~Liang, ``Human--machine shared control for path following considering driver fatigue characteristics,'' \emph{IEEE Trans. Intell. Transp. Syst.}, vol.~25, no.~7, pp.~7328--7341, 2024.
