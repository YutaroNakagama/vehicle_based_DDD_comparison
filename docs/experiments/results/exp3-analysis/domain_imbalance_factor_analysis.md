...
- **UPDATE 2026-07-12 — clean same-protocol confirmation + proba-spread evidence.** The header "0.519 → 0.791" mixes two effects (SMOTE *and* pooled→within eval). The **isolated imbalance effect** is now measured on the *identical* pooled eval: **Pooled-no-SMOTE 0.519 → Pooled+SW-SMOTE 0.724** (n=2, s0/s7; recovered by re-running eval on already-trained models — see [[project_c1_svmw_watchdog_churn]]). Non-degenerate: CM `[[7609,4258],[133,352]]`/`[[7956,3911],[141,344]]`, recall_pos ~0.72. The mechanism is now quantified at the probability level: **proba pstd 0.001 (constant, degenerate) → 0.229 (spread, discriminative)** — SMOTE restores score *spread* via minority density, not via reweighting (which was already applied doubly). Parity-with-RF caveat: seed-paired (s0/s7) RF still leads 0.793 vs 0.724 (+0.069); n=5 pending. Full method-vs-RF divergence taxonomy: [c1_results.md §3.5](c1_results.md).
...
- **Features lack signal for the KSS label.** SvmA uses steering-angle/rate statistical features (`Steering_Range … SteeringSpeed_SampleEntropy` = **39 in-range columns, 23 retained** after the paper's 14-suffix filter) with the paper-specific KSS mapping (1–6 = alert, 8–9 = drowsy, 7 excluded; `SvmA.py:SVMA_KSS_BIN_LABELS`). Direct evidence the signal is absent: pooled univariate directionless AUROC tops out at **0.515** (best feature, 0 features > 0.55) and a multivariate RBF-SVM on the 23 paper features lands at **0.509**. Balancing the classes cannot create discriminative signal that the features do not contain — so SMOTE moves the operating point but not the AUROC.
  - **Update (T1, 2026-06-27):** the 0.515/0.509 above were measured on an *unfaithful* 14-suffix filter that dropped Arefnezhad's main features (Sample Entropy, Katz FD, Shannon Entropy, Spectral Flux, Frequency Variability, Q1/Q2/Q3) and added 4 non-paper ones. The filter is now corrected to the **faithful 18 features/signal (36 cols; `SvmA.py` commit `b715535`)**. Re-testing feature signal on the faithful set (subject-disjoint split, n=66,993) gives **univariate max 0.515 (0 features > 0.55, incl. Sample Entropy 0.514), multivariate RBF-SVM 0.494–0.507, and RF on the same features 0.496** — still chance. The null is therefore **strengthened, not a feature-filter artifact, and classifier-independent** (RF, a strong learner, is also at chance on these features). See [`verification_log.md`](verification_log.md) T1/T4.
