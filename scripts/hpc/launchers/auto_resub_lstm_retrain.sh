#!/bin/bash
# ============================================================
# LSTM retraining auto-submit script (after class_weight fix)
# ============================================================
# Read unsubmitted Lstm jobs from /tmp/remaining_lstm_retrain.txt,
...
#
# Usage:
#   nohup bash scripts/hpc/launchers/auto_resub_lstm_retrain.sh &
# ============================================================
...
REMAINING_FILE="/tmp/remaining_lstm_retrain.txt"
RANKING="knn"
...
# Per-user queue limits (max_queued)
declare -A QUEUE_MAX
QUEUE_MAX[SINGLE]=40
...
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/auto_resub_lstm_retrain_${TIMESTAMP}.log"

echo "============================================================"
echo "LSTM retraining auto-submit script started: $(date)"
echo "remainingjob(s): $(wc -l < "$REMAINING_FILE") items"
echo "Retry interval: ${SLEEP_INTERVAL}s"
echo "Log: $LOG_FILE"
echo "============================================================"

echo "# LSTM retrain auto resub started at $(date)" > "$LOG_FILE"
...
while [[ -s "$REMAINING_FILE" && $RETRY -lt $MAX_RETRIES ]]; do
    ...
    # Build ordered queue list (prefer queues with more slots)
    ORDERED_QUEUES=()
    ...
    while IFS='|' read -r MODEL CONDITION MODE DISTANCE DOMAIN SEED RATIO WALLTIME MEM N_TRIALS; do
        ...
        # Find a queue with available slots (round-robin among available)
        SELECTED_QUEUE=""
        TRIED=0
        ...
        JOB_NAME="Ls_${COND_SHORT}_${DISTANCE:0:1}${DOMAIN:0:1}_${MODE_SHORT}_s${SEED}"

        CMD="qsub -N $JOB_NAME -l select=1:ncpus=4:mem=${MEM} -l walltime=${WALLTIME} -q $SELECTED_QUEUE"
...
{
    echo ""
    echo "# LSTM retrain auto resub completed at $(date)"
...
echo ""
echo "============================================================"
echo "LSTM retraining auto-submission complete: $(date)"
