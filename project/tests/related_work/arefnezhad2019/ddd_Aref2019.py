...
def calculate_feature_indices(features_df, label_df):
    ...
    indices_df = pd.DataFrame({
        ...
        'Fisher': fisher_indices,
        'Correlation': correlation_indices,
        'T_test': t_test_indices,
        'Mutual_Info': mutual_info_indices
    })
    return indices_df

# ANFISによる特徴量選択とSVM最適化
def calculate_importance_degree(params, indices_df):
    weighted_scores = (indices_df['Fisher'] * params[0] +
                       indices_df['Correlation'] * params[1] +
                       indices_df['T_test'] * params[2] +
                       indices_df['Mutual_Info'] * params[3])
...
def optimize_anfis_svm(X, y):
    def objective(params):
        anfis_params = params[:4]
        C, gamma = params[4], params[5]
        importance_degree = calculate_importance_degree(anfis_params, indices_df)
        selected_features = select_features_by_importance(importance_degree, X)
        if selected_features.shape[1] > 0:
            svm_model = SVC(kernel='rbf', C=C, gamma=gamma)
            svm_model.fit(selected_features, y)
            train_accuracy = accuracy_score(y, svm_model.predict(selected_features))
            return -train_accuracy
        else:
            return float('inf')

    bounds = [(0, 1)] * 4 + [(0.1, 10), (0.001, 1)]
    result = differential_evolution(objective, bounds, maxiter=10, popsize=3)
    return result.x
...
optimal_params = optimize_anfis_svm(features_only_df, label_df)
...
selected_features_final = select_features_by_importance(importance_degree, features_only_df)

# 訓練、検証、テストセットにデータを分割
X_train, X_temp, y_train, y_temp = train_test_split(selected_features_final, label_df, test_size=0.4, random_state=42)
...
# 各セットでの精度計算
train_accuracy = accuracy_score(y_train, svm_model_final.predict(X_train))
val_accuracy = accuracy_score(y_val, svm_model_final.predict(X_val))
test_accuracy = accuracy_score(y_test, svm_model_final.predict(X_test))
...
print("selected features\n",selected_features_final.columns.tolist())

# 重要度インデックスの長さを調整して再実行します

def select_features_by_importance_adjusted(importance_degree, features_df):
    # データフレームの列に合わせて、重要度インデックスを調整
    importance_degree_adjusted = importance_degree[:features_df.shape[1]]
    selected_features = features_df.loc[:, importance_degree_adjusted == 1]
    return selected_features

# 最適化関数内での特徴量選択部分を修正して実行します
def optimize_anfis_svm_for_val_accuracy_adjusted(X_train, y_train, X_val, y_val):
    def objective(params):
        anfis_params = params[:4]
        C, gamma = params[4], params[5]

        # インデックスに基づく重要度を計算し、出力が1の特徴量を選択
        importance_degree = calculate_importance_degree(anfis_params, indices_df)
        selected_features_train = select_features_by_importance_adjusted(importance_degree, X_train)
        selected_features_val = select_features_by_importance_adjusted(importance_degree, X_val)
        ...
        if selected_features_train.shape[1] > 0 and selected_features_val.shape[1] > 0:
            ...
            return -val_accuracy  # 検証精度最大化のため符号反転
        else:
            return float('inf')  # 特徴量がない場合、大きな値で無効にする
...
# PSOを用いた最適化
def optimize_anfis_svm_with_pso(X_train, y_train, X_val, y_val):
    def objective(params):
        anfis_params = params[:4]
        C, gamma = params[4], params[5]
        importance_degree = calculate_importance_degree(anfis_params, indices_df)
...
# 訓練と検証データに対する最適化実行
optimal_params_val_accuracy_adjusted = optimize_anfis_svm_for_val_accuracy_adjusted(X_train, y_train, X_val, y_val)

# 最適化されたパラメータでANFISとSVMを設定
best_anfis_params_val_adj = optimal_params_val_accuracy_adjusted[:4]
best_C_val_adj, best_gamma_val_adj = optimal_params_val_accuracy_adjusted[4], optimal_params_val_accuracy_adjusted[5]
importance_degree_val_adj = calculate_importance_degree(best_anfis_params_val_adj, indices_df)

# 選択された特徴量で再度評価
selected_features_train_val_adj = select_features_by_importance_adjusted(importance_degree_val_adj, X_train)
selected_features_val_val_adj = select_features_by_importance_adjusted(importance_degree_val_adj, X_val)
selected_features_test_val_adj = select_features_by_importance_adjusted(importance_degree_val_adj, X_test)

# SVMモデルの訓練と評価
svm_model_final_val_adj = SVC(kernel='rbf', C=best_C_val_adj, gamma=best_gamma_val_adj)
svm_model_final_val_adj.fit(selected_features_train_val_adj, y_train)

# 精度の計算
train_accuracy_val_adj = accuracy_score(y_train, svm_model_final_val_adj.predict(selected_features_train_val_adj))
val_accuracy_val_adj = accuracy_score(y_val, svm_model_final_val_adj.predict(selected_features_val_val_adj))
test_accuracy_val_adj = accuracy_score(y_test, svm_model_final_val_adj.predict(selected_features_test_val_adj))

# 結果の表示
print(
"result (opt for val acc)",
"\nBest ANFIS Parameters (Weights for Fisher, Correlation, T-test, Mutual Info):", best_anfis_params_val_adj,
"\nBest SVM Parameters (C, gamma)                                              :", (best_C_val_adj, best_gamma_val_adj),
"\nTrain Accuracy                                                              :", train_accuracy_val_adj,
"\nValidation Accuracy                                                         :", val_accuracy_val_adj,
"\nTest Accuracy                                                               :", test_accuracy_val_adj,
)
