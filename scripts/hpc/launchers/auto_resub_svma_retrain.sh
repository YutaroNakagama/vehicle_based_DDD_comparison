#!/bin/bash
# ============================================================
# SvmA再学習 自動投入スクリプト (class_weight / PSO F1 / 閾値最適化 修正後)
# ============================================================
# /tmp/remaining_svma_retrain.txt から未投入SvmAジョブを読み取り、
# 各キューの空き枠を確認しながら投入する。
#
# 入力形式 (パイプ区切り, 10列):
...
#
# 使い方:
#   nohup bash scripts/hpc/launchers/auto_resub_svma_retrain.sh &
# ============================================================
...
REMAINING_FILE="/tmp/remaining_svma_retrain.txt"
RANKING="knn"
...
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/auto_resub_svma_retrain_${TIMESTAMP}.log"

echo "============================================================"
echo "SvmA再学習 自動投入スクリプト 開始: $(date)"
echo "残りジョブ: $(wc -l < "$REMAINING_FILE") 件"
echo "リトライ間隔: ${SLEEP_INTERVAL}秒"
echo "ログ: $LOG_FILE"
echo "============================================================"

echo "# SvmA retrain auto resub started at $(date)" > "$LOG_FILE"
...
{
    echo ""
    echo "# SvmA retrain auto resub completed at $(date)"
