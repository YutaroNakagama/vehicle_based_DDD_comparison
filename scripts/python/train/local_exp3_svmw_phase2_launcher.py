"""Local launcher for SvmW Phase 2 — 9 additional seeds for SW-SMOTE conditions.
...
LOG_DIR = REPO / "logs" / "exp3_svmw_phase2"
...
N_WORKERS = int(os.environ.get("LOCAL_PARALLEL_SVMW", "8"))
...
def run_one(job: Job) -> int:
    ...
    env.setdefault("N_TRIALS_OVERRIDE", "100")
...
def main():
    ...
    logging.info(
        "SvmW Phase2 | Total=%d Done=%d Pending=%d Workers=%d Reverse=%s Skip=%d",
        len(all_jobs), done, len(pending), N_WORKERS, args.reverse, args.skip,
    )
    ...
    for i in range(N_WORKERS):
        t = threading.Thread(target=worker_loop, args=(f"SvmW-{i}", q), name=f"SvmW-{i}")
        t.start()
        threads.append(t)
    ...
    logging.info("All SvmW Phase 2 workers finished.")
