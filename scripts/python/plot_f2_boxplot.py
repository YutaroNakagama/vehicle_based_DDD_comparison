#!/usr/bin/env python
"""
Plot F2 Score Boxplot - Multi-seed comparison
==============================================

Creates a boxplot of F2 scores across multiple seeds for each method.
Based on the original plot_f2_standalone from performance_results.py
"""
...
def extract_results_from_evaluation(eval_dir: Path) -> pd.DataFrame:
    """Extract F2 scores and other metrics from evaluation JSON files."""
    records = []
...
def plot_f2_boxplot(
    ...
    """
    Create boxplot of F2 scores across seeds for each method.
    
    Based on plot_f2_standalone style.
    """
    # Define method order
    method_order = [
        "baseline",
    ...
    # Calculate median F2 for each method_label
    median_f2 = df.groupby("method_label")["f2_thr"].median().to_dict()
    ...
    data_for_plot = [df[df["method_label"] == m]["f2_thr"].values for m in method_labels]
    ...
    ax.set_xlabel("F2 Score", fontsize=14)
    ...
    seed_counts = df.groupby("method_label")["seed"].nunique()
...
def main():
    ...
    # Print summary
    print("\nF2 Score Summary (mean ± std):")
    print("-" * 60)
    summary = df.groupby("method_label")["f2_thr"].agg(["mean", "std", "count"])
    summary = summary.sort_values("mean", ascending=False)
    for method, row in summary.iterrows():
        print(f"{method:35s} | F2: {row['mean']:.4f} ± {row['std']:.4f} (n={int(row['count'])})")
    
    # Create boxplot
    print("\nCreating F2 Score boxplot...")
    output_path = output_dir / "f2_score_boxplot.png"
    fig = plot_f2_boxplot(df, output_path=output_path)
