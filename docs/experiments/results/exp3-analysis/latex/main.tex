...
Independent local verification on the SW-SMOTE subset ($N{=}40$ within evaluations; three distance metrics $\times$ three seeds, both domain groups, $r \in \{0.3, 0.5\}$) yields AUROC $\approx$ 0.52--0.54, consistent with the HPC aggregate (SW-SMOTE $r{=}0.1$: 0.538; $r{=}0.5$: 0.526), confirming the reliability of the HPC-sourced table values.
...
Independent local verification on the SW-SMOTE mmd in-domain subset ($N{=}23$ within evaluations: 12 seeds at $r{=}0.3$ and 11 seeds at $r{=}0.5$, retrained on RTX~3060 with cuML 25.12 GPU-accelerated SVC, kernel cache 4096~MiB) yields AUROC $= 0.486 \pm 0.007$ ($r{=}0.3$) and $0.483 \pm 0.016$ ($r{=}0.5$); the corresponding cross-domain evaluations yield AUROC $= 0.497 \pm 0.022$ and $0.501 \pm 0.020$.
These values are consistent with the HPC SW-SMOTE rows of Table~\ref{tab:svma_results} (within-domain AUROC $\approx$ 0.499--0.500), confirming that the degenerate behaviour is implementation-independent and reproducible across the sklearn~1.7.2 / libsvm CPU and cuML~25.12 GPU back-ends.
...
\begin{table}[!t]
\caption{\Impl{Lstm} Within-Domain Performance (SW-SMOTE conditions, locally verified, F1 threshold). Averaged over three distance metrics $\times$ three seeds per cell ($N{=}9$ per row, fully covered). Training-domain group (in-/out-domain) denotes the domain used for training; within-domain evaluation on matched subjects. N/A: ratio $r{=}0.1$ infeasible for DRT labels (natural minority prevalence $\approx$27\%). Baseline, RUS, and SMOTE conditions: AUROC $\approx$ 0.77--0.78 from prior HPC aggregate (threshold-independent; threshold-sensitive metrics not available as post-fix data).\label{tab:lstm_results}}
\centering
\footnotesize
...
SW-SMOTE $r\!=\!0.5$ & in-dom. & 9 & 0.914 & 0.802 & 0.926 & 0.965 & 0.757 \\
\hline
SW-SMOTE $r\!=\!0.3$ & out-dom. & 9 & 0.911 & 0.740 & 0.896 & 0.961 & 0.754 \\
SW-SMOTE $r\!=\!0.5$ & out-dom. & 9 & 0.916 & 0.710 & 0.886 & 0.971 & 0.746 \\
\hline
Baseline / RUS / SMOTE & -- & -- & \multicolumn{5}{c|}{AUROC $\approx$ 0.77--0.78 (HPC aggregate); threshold-sensitive metrics pending} \\
RUS / SMOTE $r\!=\!0.1$ & -- & -- & \multicolumn{5}{c|}{N/A} \\
\hline
\end{tabular}
\end{table}

Table~\ref{tab:lstm_results} shows locally verified \Impl{Lstm} performance for SW-SMOTE conditions (post-fix, F1 threshold, $\beta{=}1.0$; $N{=}36$ within-domain evaluations, fully covered: all three distance metrics $\times$ three seeds per cell).
Training-domain group strongly modulates AUROC: in-domain training achieves AUROC $\approx$ 0.82 (SW-SMOTE $r{=}0.3$) versus $\approx$ 0.72--0.74 for out-domain, reflecting the expected benefit of matching training and test domains.
...
AUROC $\approx$ 0.82 (in-domain) and $\approx$ 0.72--0.74 (out-domain) reflect genuine rank discrimination that is substantially better than \Impl{SvmA} (AUROC $\approx$ 0.50) and moderately better than \Impl{SvmW} (AUROC $\approx$ 0.556), yet still 0.07 below RF (AUROC 0.890).
...
The LSTM architecture achieves genuine discrimination (best AUROC $\approx$ 0.82 in-domain, $\approx$ 0.72--0.74 out-domain) but consistently predicts a large fraction of alert windows as drowsy (recall $\approx$ 0.98, FP rate $\approx$ 88\% on alert windows).
...
\begin{enumerate}
\item \textbf{Partial Lstm rebalancing coverage.} Locally verified \Impl{Lstm} results cover SW-SMOTE conditions with full coverage ($N{=}36$: all three distance metrics $\times$ three seeds $\times$ both domain groups $\times$ two ratios). Baseline, RUS, and SMOTE conditions remain pending from HPC; their AUROC $\approx$ 0.77--0.78 from a prior aggregate is threshold-independent and directionally consistent with local results, but threshold-sensitive metrics (recall, F2) from that aggregate are stale (pre-fix, F2 threshold). The positive-class bias pattern is consistent across all 36 locally verified runs.
