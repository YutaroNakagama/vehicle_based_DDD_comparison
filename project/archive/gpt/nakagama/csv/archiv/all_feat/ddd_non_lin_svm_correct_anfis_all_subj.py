...
# Load the list of subject data paths
with open('../../../../../../dataset/Aygun2024/subject_list.txt') as f:
    subject_files = [line.strip() for line in f.readlines()]

# Define a function to load and process each subject's data
def load_and_process_data(subject_files):
    datasets = []
    for path in subject_files:
        # Split path into subject_id and version based on the format {subject_id}/{subject_id}_{version}
        subject_id, version = path.split('/')[-2], path.split('/')[-1]
        # Construct the correct file path based on the expected pattern
        full_path = f'./merged_data_table_{version}.csv'
        
        # Check if file exists; if not, skip to the next
...
datasets = load_and_process_data(subject_files)
