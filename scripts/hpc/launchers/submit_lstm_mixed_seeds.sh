#!/bin/bash
# ============================================================
# Submit Lstm MODE=mixed jobs for all 15 seeds (none currently
# valid — earlier mixed runs were invalidated).
#
# Per seed: 7 cond-variants × 3 dist × 2 dom = 42 jobs
# Total: 15 seeds × 42 = 630 jobs (GPU)
# ============================================================
set -uo pipefail
...
submit_one() {
    ...
    local JOB_NAME="Ls_${CS}_${DL}${DM}_mx${RTAG}_s${SEED}"
    ...
    local QUEUE="${GPU_QUEUES[$((GPU_IDX % ${#GPU_QUEUES[@]}))]}"
    ((GPU_IDX++))

    local CMD="qsub -N $JOB_NAME -l select=1:ncpus=4:ngpus=1:mem=32gb -l walltime=24:00:00 -q $QUEUE"
