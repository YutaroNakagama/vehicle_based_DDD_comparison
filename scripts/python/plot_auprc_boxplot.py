#!/usr/bin/env python
"""
Plot AUPRC Boxplot - Multi-seed comparison
===========================================

Creates a boxplot of AUPRC (Area Under Precision-Recall Curve) across multiple seeds for each method.
Based on plot_precision_boxplot.py
"""
...
def extract_results_from_evaluation(eval_dir: Path) -> pd.DataFrame:
    """Extract AUPRC and other metrics from evaluation JSON files."""
    records = []
...
def plot_auprc_boxplot(
    df: pd.DataFrame,
    figsize: tuple = (14, 12),
    output_path: Path = None,
) -> plt.Figure:
    """
    Create boxplot of AUPRC scores across seeds for each method.
    """
    # Define method order
    method_order = [
        "baseline",
    ...
    data_for_plot = [df[df["method_label"] == m]["auprc"].values for m in method_labels]
    ...
    for i, (data, label) in enumerate(zip(data_for_plot, method_labels)):
        if len(data) > 0:
            med = np.median(data)
            ax.text(med + 0.005, i + 1, f"{med:.3f}", 
...
def main():
    ...
    # Print summary
    print("\nAUPRC Summary (mean ± std):")
    print("-" * 60)
    summary = df.groupby("method_label")["auprc"].agg(["mean", "std", "count"])
    summary = summary.sort_values("mean", ascending=False)
    for method, row in summary.iterrows():
        print(f"{method:35s} | AUPRC: {row['mean']:.4f} ± {row['std']:.4f} (n={int(row['count'])})")
    
    # Create boxplot
    print("\nCreating AUPRC boxplot...")
    output_path = output_dir / "auprc_boxplot.png"
    fig = plot_auprc_boxplot(df, output_path=output_path)
