...
def xgb_predict(NUM_DATA_local, yawrate_i_filt_local, yawrate_a_filt_local, NUM_RNN_local, regressor_local):

    y_test = np.array(yawrate_a_filt_local[0:NUM_RNN]).reshape(-1)
    x_test = np.array(yawrate_i_filt_local[0:NUM_RNN]).reshape(-1)
    ...
    for j in (range(NUM_DATA_local)):
        ...
        if input_omega == "omega_hat": 
            x1_test = np.array(x_test[j:j+NUM_RNN])
        else:
            #x1_test = np.array(yawrate_a_filt_local[j:j+NUM_RNN]).reshape(1, NUM_RNN, 1) 
            x1_test = np.array(yawrate_i_filt_local[j:j+NUM_RNN])#.reshape(1, NUM_RNN, 1) 
        ...
        y_pred = regressor_local.predict(pd.DataFrame(x1_test))#.numpy()
        elapsed_time = time.time() - fit_start
        #print(f"fit elapsed time: {elapsed_time}")

        if input_omega == "omega_hat": 
            y_test = np.concatenate([y_test, y_pred[-1].reshape(-1)],0) 
        else:
            y_test = np.concatenate([y_test, y_pred[step].reshape(-1)],0) 
        #y_test = np.concatenate([y_test, y_pred[NUM_RNN-1].reshape(-1)],0) 
        x_test = np.concatenate([x_test, x1_test[NUM_RNN-1].reshape(-1)],0) 
        #plt.plot(y_test)
        #plt.show()
...
def train_focused_data(regressor, freq_th):
...
#                                                )
                ...
                fit_start = time.time()
                regressor.fit(pd.DataFrame(yawrate_i_filt), pd.DataFrame(yawrate_a_filt))
...
#                           +rnn_type+'_'+str(NUM_DIM)+'_'+str(n_batch)+'_'+current_date+'.h5')
                ...
                save_eg_csv = True
                if save_eg_csv:
                    x_test, y_test = xgb_predict(NUM_DATA, yawrate_i_filt, yawrate_a_filt, NUM_RNN, regressor)
                    csv_df = pd.DataFrame(
                    ...
                    csv_path = "../result/csv/xgb/predict_data_"+str(step)+"step_"+\
                                current_time+"_"+input_omega+".csv"
...
#                                                )
#
                #regressor.fit(yawrate_i_filt, yawrate_a_filt)
                regressor.fit(pd.DataFrame(yawrate_i_filt), pd.DataFrame(yawrate_a_filt))
#                                    epochs=n_epoch,\
#                                    batch_size=n_batch,\
...
#            loss = history.history['loss']
            if save_fig_loss_flg: save_loss_fig(loss, i, freq_th, eeg_high_th_str,\
        ...
        if save_fig_fft: 
            draw_fig_fft(time_str, yawrate_i_filt, yawrate_a_filt,\
...
def test_datasets(best_regressor,freq_th):
...
#                   +rnn_type+'_'+str(NUM_DIM)+'_'+str(n_batch)+'_'+current_date+'.h5')
        ...
        y_pred, y_test = xgb_predict(NUM_DATA, yawrate_i_filt+1, yawrate_a_filt+1, NUM_RNN, best_regressor)
        ...
        csv_path = "../result/csv/xgb/rmse_fc_"+str(step)+"step_"\
                    +"_"+current_date+"_"+input_omega+".csv"
...
def objective(trial):
    ...
    NUM_DIM  = trial.suggest_int("NUM_DIM",5,100)
    rnn_type = trial.suggest_categorical("rnn_type",["LSTM","RNN","GRU"])
    n_batch  = trial.suggest_categorical("n_batch",[16,32,64])
    """
    #freq_th  = trial.suggest_int("freq_th",4,6)
    freq_th  = 4
    params = {'n_estimators' : 
...
#    regressor = xgb.XGBRegressor()
    ...
    best_regressor = train_focused_data(regressor, freq_th)

    # velidation
    # auc_score  = test_datasets(best_model,NUM_DIM, rnn_type, n_batch, freq_th)
    auc_score  = test_datasets(best_regressor, freq_th)
