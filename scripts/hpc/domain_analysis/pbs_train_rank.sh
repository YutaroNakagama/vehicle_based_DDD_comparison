...
# Reduce N_TRIALS to avoid memory issues during Optuna optimization
...
#RANK_NAMES_FILE="$PROJECT_ROOT/results/domain_analysis/distance/random_names.txt"
IDX="${PBS_ARRAY_INDEX:-1}"
GROUP_FILE="$(sed -n "${IDX}p" "$RANK_NAMES_FILE" | tr -d '\r\n')"
BASENAME="$(basename "$GROUP_FILE" .txt)"
TAG="rank_${BASENAME}"

echo "[INFO] Training $MODEL for $TAG (index=$IDX)"
