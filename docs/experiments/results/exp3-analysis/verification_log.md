...
## 2026-07-26 [SvmA GPU cost + robustness fix] iv25smote SW-SMOTE is ~9 h/seed; moved GPU work to a Scheduled Task

The SvmA iv25smote SW-SMOTE regeneration is far slower than the original pooled-SMOTE fallback (~2.8 h/seed): subject-wise SMOTE inflates the training set to 57k samples, so each cuML-GPU PSO/SVM seed takes ~9 h (s0 8.5 h, s1 9.7 h). Only 2/6 completed before all three run_in_background WSL jobs died together (exit 1 = WSL session-recycling fragility; WSL and the venv were themselves healthy). Fix: the SvmA GPU work (finish the 4 remaining regen seeds, then run the c1 +3-seed augmentation #4) now runs under a Windows Scheduled Task `SvmA_GPU_Finish` (7-day time limit, 3 restarts) which survives the session and WSL recycling; `already_done` makes re-runs resume. Cost note: remaining SvmA GPU work is ~36 h (regen) + ~2 days (#4) = ~3-4 days for a method that is chance either way; the CPU augmentations (#2 RF-nofs, #3 SvmW) are unaffected and running normally.
