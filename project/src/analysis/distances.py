# project/src/analysis/distances.py
...
# ======== Core helpers (moved from bin/comp_dist.py) ========
...
def compute_mmd(X: np.ndarray, Y: np.ndarray, gamma: float | None = None) -> float:
    """Biased MMD^2 estimate with RBF kernel (matches prior behavior)."""
    if gamma is None:
...
def _extract_features(subjects: list[str], data_root: Path) -> dict[str, np.ndarray]:
    """Load features for all subjects, skipping missing ones."""
...
# ======== Orchestrator (callable from bin) ========
...
def run_comp_dist(
    ...
    """Entry point equivalent to old bin/comp_dist.py, but parametrized.

    All arguments have defaults matching the previous hard-coded paths for backward compatibility.
    """
    # 1) Load subjects and features
    subjects = _load_subject_list(Path(subject_list_path))
    features = _extract_features(subjects, Path(data_root))

    # Global z-score across all subjects (as before)
    if features:
    ...
    # 4) Per-distance postprocessing and group summaries
    ...
    for dist_name, (matrix_path, subject_path) in distance_types.items():
        ...
        subjects = json.loads(Path(subject_path).read_text())
        ...
        _plot_group_projection(matrix, subjects, groups, dist_name,
        ...
        intra_results = _compute_intra_group_variability(matrix, subjects, groups)
        intra_dir = group_dir / "intra"; intra_dir.mkdir(parents=True, exist_ok=True)
        (intra_dir / "intra_group_variability.json").write_text(json.dumps(intra_results, indent=2))
        pd.DataFrame(intra_results).T.to_csv(intra_dir / "intra_group_variability.csv")
        _plot_bar(list(intra_results.keys()),
                  np.array([intra_results[g]["mean"] for g in intra_results]),
                  np.array([intra_results[g]["std"] for g in intra_results]),
                  f"Intra-group Variability - {dist_name}", "Mean Distance",
                  intra_dir / "intra_variability.png")

        stats = _compute_intra_inter_stats(matrix, subjects, groups)
