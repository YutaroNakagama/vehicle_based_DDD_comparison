#!/bin/bash
# ============================================================
# Submit ONLY missing jobs (skip already submitted)
# Based on analysis of job_ids_multiseed_ratio_*.txt
# ============================================================
...
JOB_LOG="$PROJECT_ROOT/scripts/hpc/logs/imbalance/job_ids_missing_$(date +%Y%m%d_%H%M%S).txt"
echo "# Missing job submission: $(date)" > "$JOB_LOG"
...
submit_job() {
    ...
    if [[ "$RATIO" == "default" ]]; then
        # Fixed-ratio method (baseline, balanced_rf, easy_ensemble)
    ...
    TRAIN_ID="${TRAIN_JOB%%.*}"
    echo "  [$METHOD] seed=$SEED ratio=$RATIO q=$QUEUE -> $TRAIN_ID"
...
# ============================================================
# Missing jobs based on analysis:
# 
# Fixed-ratio (already submitted but baseline was in TINY - need resubmit):
# - baseline: seed 42,123,456 - RESUBMIT with DEFAULT queue
#
# Variable-ratio seed=42 ratio=1.0 missing:
# - smote_enn, smote_rus, smote_balanced_rf
# - undersample_rus, undersample_tomek, undersample_enn
#
# Variable-ratio seed=123: ALL methods, ALL ratios
# Variable-ratio seed=456: ALL methods, ALL ratios
# ============================================================

echo ""
echo "=== Re-submit baseline (was TINY, now DEFAULT) ==="
for SEED in 42 123 456; do
    submit_job "baseline" "$SEED" "default"
done

echo ""
echo "=== seed=42, ratio=1.0 missing methods ==="
...
echo ""
echo "=== seed=123: ALL variable-ratio methods ==="
...
echo ""
echo "=== seed=456: ALL variable-ratio methods ==="
for RATIO in 0.1 0.5 1.0; do
    echo "  -- ratio=$RATIO --"
    for METHOD in smote smote_tomek smote_enn smote_rus smote_balanced_rf undersample_rus undersample_tomek undersample_enn; do
