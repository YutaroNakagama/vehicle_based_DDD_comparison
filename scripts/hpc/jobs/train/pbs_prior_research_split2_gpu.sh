...
# Issue #12 (2026-05-08): XLA JIT (used by TF for ops like Sign in Lstm metrics)
# requires libdevice.10.bc which is shipped under <CUDA>/nvvm/libdevice/.
# The cuda/12.8u1 module sets CUDA_HOME on compute nodes (/app/CUDA/12.8u1);
# fall back to the kagayaki mirror if that env is missing.
if [[ -z "${XLA_FLAGS:-}" ]]; then
