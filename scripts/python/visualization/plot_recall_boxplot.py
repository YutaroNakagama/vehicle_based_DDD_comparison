#!/usr/bin/env python
"""
Plot Recall Boxplot - Multi-seed comparison
============================================

Creates a boxplot of Recall (Drowsy Detection Rate) across multiple seeds for each method.
Based on plot_f2_boxplot.py
"""
...
def extract_results_from_evaluation(eval_dir: Path) -> pd.DataFrame:
    """Extract Recall and other metrics from evaluation JSON files."""
    records = []
...
def plot_recall_boxplot(
    ...
    """
    Create boxplot of Recall scores across seeds for each method.
    """
    # Define method order
    method_order = [
        "baseline",
    ...
    # Calculate median Recall for each method_label
    median_recall = df.groupby("method_label")["recall_thr"].median().to_dict()
    ...
    data_for_plot = [df[df["method_label"] == m]["recall_thr"].values for m in method_labels]
    ...
    for i, (data, label) in enumerate(zip(data_for_plot, method_labels)):
        if len(data) > 0:
            med = np.median(data)
            ax.text(med + 0.01, i + 1, f"{med*100:.1f}%", 
                    va="center", fontsize=9, fontweight="bold")
    
    ax.set_xlabel("Recall (Drowsy Detection Rate)", fontsize=14)
...
def main():
    ...
    # Print summary
    print("\nRecall Summary (mean ± std):")
    print("-" * 60)
    summary = df.groupby("method_label")["recall_thr"].agg(["mean", "std", "count"])
    summary = summary.sort_values("mean", ascending=False)
    for method, row in summary.iterrows():
        print(f"{method:35s} | Recall: {row['mean']*100:.1f}% ± {row['std']*100:.1f}% (n={int(row['count'])})")
    
    # Create boxplot
    print("\nCreating Recall boxplot...")
    output_path = output_dir / "recall_boxplot.png"
    fig = plot_recall_boxplot(df, output_path=output_path)
