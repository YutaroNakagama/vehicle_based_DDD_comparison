...
Third, \Impl{Lstm} recall-saturates (recall $\approx$ 1.0, F2 $\approx$ 0.93 driven by the class prior, AUROC $\approx$ 0.78), showing genuine but modest discrimination (AUROC 0.78 vs.\ 0.890 for RF) while appearing deceptively strong on F2.
These findings confirm that RF's advantage reported in the prior comparison study is genuine and was understated: the controlled AUROC gap between RF and the best prior method grows from 0.04 (uncontrolled) to $\geq$0.33 (controlled).
...
\item We show that controlled evaluation exposes three distinct and interpretable method-level failure modes: (i)~PSO-based adaptive feature selection collapses to a trivial predictor in \Impl{SvmA}; (ii)~event-based labeling combined with an LSTM architecture leads to recall saturation in \Impl{Lstm}; and (iii)~\Impl{SvmW} partially responds to rebalancing but remains substantially below RF.

\item We demonstrate that RF's advantage from~\cite{nakagama2025iv} was genuine and understated: the controlled AUROC gap between RF and the best prior method increases from 0.04 (uncontrolled setting, AUROC 0.85 vs.\ 0.81) to $\geq$0.33 (controlled setting, AUROC 0.890 vs.\ 0.556), confirming that the prior comparison's conclusion was qualitatively correct but quantitatively conservative.
\end{enumerate}
...
Table~\ref{tab:svma_results} shows \Impl{SvmA} performance.
Across \emph{all} rebalancing conditions, distance metrics, domain groups, and seeds, \Impl{SvmA} collapses to a near-random predictor: AUROC $\approx 0.499$, F2 $\approx 0.159$, AUPRC $\approx 0.037$.
The F2 value closely matches the class prior (drowsy prevalence $\approx$ 16\%), suggesting the model predicts nearly all windows as drowsy (recall $\approx 0.92$, precision $\approx 0.037$).
...
\begin{table}[!t]
\caption{\Impl{SvmA} Performance Under Within-Domain Training. Averaged over $D$, $G$, 15 seeds. All conditions degenerate to near-random (AUROC $\approx$ 0.50); values are practically identical across rebalancing strategies.\label{tab:svma_results}}
\centering
\footnotesize
\begin{tabular}{|l|c|c|c|c|c|}
\hline
Rebalancing & F2 & AUROC & AUPRC & Recall & Precision \\
\hline
Baseline & 0.159 & 0.499 & 0.037 & 0.931 & 0.037 \\
\hline
RUS $r\!=\!0.1$ & 0.159 & 0.499 & 0.037 & 0.906 & 0.037 \\
RUS $r\!=\!0.5$ & 0.158 & 0.500 & 0.039 & 0.905 & 0.037 \\
\hline
SMOTE $r\!=\!0.1$ & 0.159 & 0.500 & 0.037 & 0.911 & 0.037 \\
SMOTE $r\!=\!0.5$ & 0.159 & 0.501 & 0.038 & 0.885 & 0.037 \\
\hline
SW-SMOTE $r\!=\!0.1$ & 0.159 & 0.499 & 0.037 & 0.896 & 0.037 \\
SW-SMOTE $r\!=\!0.5$ & 0.159 & 0.500 & 0.038 & 0.911 & 0.037 \\
\hline
\end{tabular}
\end{table}
...
\begin{table}[!t]
\caption{\Impl{Lstm} Performance Under Within-Domain Training (preliminary; 30\% coverage). Averaged over $D$, $G$, available seeds, within-domain eval. N/A cells: ratio $r{=}0.1$ infeasible for DRT labels.\label{tab:lstm_results}}
\centering
\footnotesize
\begin{tabular}{|l|c|c|c|c|c|}
\hline
Rebalancing & F2 & AUROC & AUPRC & Recall & Prec. \\
\hline
Baseline & 0.932 & 0.779 & 0.913 & 0.999 & 0.735 \\
\hline
RUS $r\!=\!0.1$ & N/A & N/A & N/A & N/A & N/A \\
RUS $r\!=\!0.5$ & 0.933 & 0.783 & 0.915 & 0.999 & 0.736 \\
\hline
SMOTE $r\!=\!0.1$ & N/A & N/A & N/A & N/A & N/A \\
SMOTE $r\!=\!0.5$ & 0.933 & 0.773 & 0.910 & 0.999 & 0.738 \\
\hline
SW-SMOTE $r\!=\!0.1$ & 0.933 & 0.782 & 0.915 & 0.999 & 0.737 \\
SW-SMOTE $r\!=\!0.5$ & 0.932 & 0.754 & 0.904 & 1.000 & 0.734 \\
\hline
\end{tabular}
\end{table}

Table~\ref{tab:lstm_results} shows \Impl{Lstm} performance (preliminary; 30\% of the target 15-seed coverage).
The LSTM model exhibits \emph{recall saturation}: recall $\approx 1.000$ and precision $\approx 0.735$ across all feasible conditions.
With DRT-based labels ($\approx$27\% positive-class prevalence), a model that predicts all windows as positive would achieve precision = 0.735 and recall = 1.0, yielding F2 = $5 \times 0.735/(4 \times 0.735 + 1) = 0.93$---precisely matching the observed values.
The F2 score of $\approx$0.932 is therefore driven entirely by the class prior, not by discrimination.

AUROC $\approx$ 0.77--0.78, by contrast, reflects genuine (if modest) rank discrimination.
The discrepancy between AUROC and F2 signals that the model generates a scored ranking that is better than random, but the calibrated threshold collapses to predict-all-positive when F2 is used for threshold selection (because F2 emphasizes recall, predicting all positive maximizes F2 under imbalance).

Contrasting with \Impl{SvmA}'s collapse to near-random (AUROC $\approx$ 0.50), \Impl{Lstm}'s AUROC of 0.78 indicates a genuinely more capable scorer; the failure mode is not in the model's discrimination but in its threshold-selection interaction with F2 optimization and high class prior.
Comparing with~\cite{nakagama2025iv}, where \Impl{Lstm} under configuration C2 (8:1:1 random split) collapsed in the \emph{opposite} direction (AUROC = 0.52, recall = 5\%, model predicts all-negative), the within-domain evaluation here reveals a qualitatively different but equally degenerate behavior: the model saturates recall rather than collapsing it, reflecting sensitivity to the training distribution shift between random pooling (C2) and within-domain partitioning (Experiment~3).
...
RF (from~\cite{nakagama2026tiv}) achieves the highest values across all three metrics.
The AUROC ranking is RF (0.890) $\gg$ \Impl{Lstm} (0.779, recall-saturated) $>$ \Impl{SvmW} (0.556) $\gg$ \Impl{SvmA} (0.499).
When AUROC is used as the primary metric (which is immune to threshold-selection artifacts), the RF gap over the best prior method (\Impl{Lstm}) is 0.111 and over \Impl{SvmW} is 0.334.
...
\hline
\Impl{Lstm}~\cite{wang2022} & Baseline & 0.932 & 0.779 & 0.913 & $\approx$1.000 & 0.735 & Recall saturation (F2 = class prior) \\
\hline
\Impl{SvmW}~\cite{zhao2012} & RUS $r\!=\!0.5$ & 0.347 & 0.556 & 0.119 & 0.955 & 0.100 & Wavelet features insufficient for fine class sep. \\
\hline
\Impl{SvmA}~\cite{arefnezhad2019} & (any; all equiv.) & $\approx$0.159 & $\approx$0.499 & $\approx$0.037 & 0.931 & 0.037 & PSO-ANFIS degeneration to trivial predictor \\
\hline
\end{tabular}
\end{table*}
...
\begin{figure}[!t]
\centering
\todo{generate: bar chart with four method groups (RF, Lstm, SvmW, SvmA), three bars per group (F2, AUROC, AUPRC), each method at its best-rebalancing condition; annotate Lstm F2 as ``recall-saturated'' and SvmA AUROC with ``degenerate''}
...
\Impl{Lstm}'s high F2 (0.932) reflects recall saturation, not superior discrimination (AUROC = 0.779); \Impl{SvmA}'s F2 (0.159) reflects class prior, not valid detection (AUROC $\approx$ 0.50).}
...
\hline
\Impl{Lstm} & 0.52 (recall = 5\%) & -- & 0.779$^{\dag}$ & Opposite degeneration (recall collapse $\to$ saturation) \\
\hline
\multicolumn{5}{|p{0.97\textwidth}|}{\footnotesize $^{\dag}$\,AUROC 0.779 reflects genuine rank discrimination, but F2 = 0.932 is driven by recall saturation and does not represent valid drowsiness detection performance.} \\
\hline
\end{tabular}
\end{table*}
...
Third, \textbf{\Impl{Lstm} shows opposite degenerations}: in the uncontrolled setting (recall = 5\%) the model collapsed to predict-all-negative; in the controlled within-domain setting (recall $\approx$1.0) it collapses to predict-all-positive.
The AUROC improvement from 0.52 to 0.779 reflects a genuine improvement in rank discrimination under within-domain training, confirming that the LSTM architecture is capable of meaningful feature learning---but the F2-optimized threshold consistently degenerates to a trivial extreme because F2 strongly rewards recall under the positive-class imbalance.
...
\Impl{Lstm}'s AUROC of 0.779 (controlled) vs.\ 0.52 (uncontrolled) suggests it is the method most sensitive to evaluation conditions.
However, because this AUROC reflects recall saturation rather than balanced discrimination (Section~\ref{sec:lstm_results}), \Impl{Lstm} cannot be considered competitive with RF for practical DDD deployment despite its higher AUROC rank compared with \Impl{SvmW}.
...
\textbf{Lstm: F2-threshold saturation.}
The LSTM architecture achieves genuine discrimination (AUROC $\approx$ 0.78) but the F2-optimized threshold collapses to predict-all-positive.
Under DRT-based labels with $\approx$27\% positive prevalence and $\beta = 2$ recall weighting in F2, the F2-maximizing threshold on the validation set is typically at or near zero (predict everything positive), yielding recall = 1.0 and precision $\approx$ 0.735 (class prior).
This is not a deficiency of the LSTM architecture but of the threshold optimization criterion: F2 with high $\beta$ under moderate imbalance incentivizes recall saturation.
A precision-oriented or threshold-independent criterion (e.g., AUROC or AUPRC) would reveal the LSTM's genuine but limited discrimination without triggering the threshold collapse.
...
\item \textbf{\Impl{Lstm}}: Replace F2 threshold optimization with AUROC-based or AUPRC-based threshold selection to avoid recall saturation; alternatively, use a fixed threshold based on the validation set precision--recall curve.
\end{enumerate}
...
\begin{enumerate}
\item \textbf{Preliminary Lstm coverage.} At time of writing, \Impl{Lstm} results cover 30\% of the target 15-seed scope (190 of 630 evaluations). The recall-saturation pattern is consistent and statistically clear, but the absolute AUROC estimates may shift slightly as additional seeds complete.
...
First, RF's advantage over prior methods is genuine and understated: the controlled AUROC gap between RF and the best discriminating prior method (\Impl{SvmW}: 0.556) is 0.334, growing from the 0.04 improvement over the uncontrolled baseline.

Second, each prior method exhibits a distinct and interpretable failure mode under controlled conditions: PSO-ANFIS optimization degeneration in \Impl{SvmA} (AUROC $\approx$ 0.50, invariant to rebalancing); F2-threshold saturation in \Impl{Lstm} (recall $\approx$ 1.0, F2 $\approx$ 0.93 driven by class prior, AUROC 0.779); and feature-space insufficiency in \Impl{SvmW} (AUROC 0.556, responding only to under-sampling).
