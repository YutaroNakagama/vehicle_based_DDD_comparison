...
if __name__ == '__main__':
    ...
    veh_eeg_all = dataset.load_veh_eeg(data_type="full")
    ...
    epochs = 2000 
    batch_size = 16
    test_size = 0.10 # % of test datasets (train_val (1-size) vs. test (size))
    ...
    drow_inputs = veh_eeg_inputs[veh_eeg_labels[:,2] < beta_lo_thld,:,:2]
    drow_labels = veh_eeg_labels[veh_eeg_labels[:,2] < beta_lo_thld,:2]
    drow_times  = veh_eeg_times[veh_eeg_labels[:,2] < beta_lo_thld]
    awake_inputs = veh_eeg_inputs[veh_eeg_labels[:,2] > beta_hi_thld,:,:2]
    awake_labels = veh_eeg_labels[veh_eeg_labels[:,2] > beta_hi_thld,:2]
    awake_times  = veh_eeg_times[veh_eeg_labels[:,2] > beta_hi_thld]
